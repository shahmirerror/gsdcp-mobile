...
export type Country = {
  ...
  dialCode: string; // E.164 country calling code, e.g. "+92"
};

// Comprehensive list of countries with their E.164 calling codes, sorted by name.
...
const BY_ISO: Record<string, Country> = COUNTRIES.reduce(
  (acc, c) => {
    acc[c.iso2] = c;
    return acc;
  },
...
// The club is Pakistan-based, so default there when the device region is unknown.
export const DEFAULT_COUNTRY: Country = BY_ISO.PK;

export function countryByIso(iso2?: string | null): Country | undefined {
  if (!iso2) return undefined;
  return BY_ISO[iso2.toUpperCase()];
}

/**
 * Best-effort 2-letter device region from the OS locale (e.g. "PK", "US").
 * Uses the locale React Native already exposes — no extra dependency and no
 * runtime permission. Returns null if it can't be determined.
 */
export function getDeviceRegion(): string | null {
  try {
    ...
    // Locale looks like "en_PK", "en-PK", "ur_PK#u-...": pull the region part.
    const match = locale.replace(/-/g, "_").match(/_([A-Za-z]{2})\b/);
...
/** Country to pre-select for phone entry: device region if known, else Pakistan. */
export function getInitialCountry(): Country {
  return countryByIso(getDeviceRegion()) ?? DEFAULT_COUNTRY;
}
